# Preprocessing summary -- horizon 24 h
...
Source: `data/raw/beijing_multisite.csv` -> `data/processed/h24/`.

Window **24 h**, horizon **24 h**, split
...
| 5. windows -> train | 293,976 | shape (293976, 24, 9) |
| 5. windows -> val | 62,544 | shape (62544, 24, 9) |
| 5. windows -> test | 62,556 | shape (62556, 24, 9) |
...
| --- | --- | --- | --- | --- |
| `is_imputed` | label row (t+h) had any modelled channel forward-filled | 20,427 | 1,724 | 2,261 |
| `is_imputed_pm25` | label row had **PM2.5** forward-filled -- this sets the class | 6,160 | 1,294 | 1,213 |
| `is_imputed_input` | any of the 24 input hours was forward-filled | 91,600 (31.2%) | 14,749 (23.6%) | 14,855 (23.8%) |
...
PM2.5, of which **8,667** become the label row of a
sample (**24,499** / **24,412**
for any modelled channel). The gap is the
1,692 boundary rows per station per split that can never
...
prediction target (t + 24 h) of every sample.

| Class | Overall | train n (%) | val n (%) | test n (%) |
| --- | --- | --- | --- | --- |
| Good | 15.23% | 41,050 (13.96%) | 11,370 (18.18%) | 11,200 (17.90%) |
| Moderate | 22.03% | 63,917 (21.74%) | 14,292 (22.85%) | 14,159 (22.63%) |
| Unhealthy (sensitive) | 12.91% | 38,438 (13.08%) | 9,069 (14.50%) | 6,565 (10.49%) |
| Unhealthy | 34.55% | 104,850 (35.67%) | 20,878 (33.38%) | 19,208 (30.71%) |
| **Very unhealthy** | 10.72% | 33,032 (11.24%) | 4,255 (6.80%) | 7,628 (12.19%) |
| **Hazardous** | 4.56% | 12,689 (4.32%) | 2,680 (4.28%) | 3,796 (6.07%) |
...
47 boundary hours per station per split that cannot complete a
window (1,692 samples in total).
...
(6.80%) and test (12.19%)** -- a 1.8x
gap, and validation also sits below its own training share
(11.24%). This is a direct consequence of the
...
| `data/processed/tabular_<split>.csv` | same samples, flat -- the window's last step plus the t+24 h label; baseline model and CTGAN input. Carries `is_imputed`, `is_imputed_pm25`, `is_imputed_input` as trailing columns |
...
Reproduce this horizon with `python -m src.preprocessing.pipeline --horizon 24`,
