# Preprocessing summary -- horizon 1 h
...
Source: `data/raw/beijing_multisite.csv` -> `data/processed/h1/`.

Window **24 h**, horizon **1 h**, split
...
| 5. windows -> train | 294,252 | shape (294252, 24, 9) |
| 5. windows -> val | 62,820 | shape (62820, 24, 9) |
| 5. windows -> test | 62,832 | shape (62832, 24, 9) |
...
| --- | --- | --- | --- | --- |
| `is_imputed` | label row (t+h) had any modelled channel forward-filled | 20,433 | 1,724 | 2,295 |
| `is_imputed_pm25` | label row had **PM2.5** forward-filled -- this sets the class | 6,160 | 1,294 | 1,247 |
| `is_imputed_input` | any of the 24 input hours was forward-filled | 91,708 (31.2%) | 14,805 (23.6%) | 15,000 (23.9%) |
...
PM2.5, of which **8,701** become the label row of a
sample (**24,499** / **24,452**
for any modelled channel). The gap is the
864 boundary rows per station per split that can never
...
prediction target (t + 1 h) of every sample.

| Class | Overall | train n (%) | val n (%) | test n (%) |
| --- | --- | --- | --- | --- |
| Good | 15.23% | 41,146 (13.98%) | 11,370 (18.10%) | 11,315 (18.01%) |
| Moderate | 22.03% | 64,007 (21.75%) | 14,292 (22.75%) | 14,274 (22.72%) |
| Unhealthy (sensitive) | 12.91% | 38,480 (13.08%) | 9,069 (14.44%) | 6,602 (10.51%) |
| Unhealthy | 34.55% | 104,898 (35.65%) | 20,982 (33.40%) | 19,216 (30.58%) |
| **Very unhealthy** | 10.72% | 33,032 (11.23%) | 4,424 (7.04%) | 7,629 (12.14%) |
| **Hazardous** | 4.56% | 12,689 (4.31%) | 2,683 (4.27%) | 3,796 (6.04%) |
...
24 boundary hours per station per split that cannot complete a
window (864 samples in total).
...
| `data/processed/tabular_<split>.csv` | same samples, flat -- the window's last step plus the t+1 h label; baseline model and CTGAN input. Carries `is_imputed`, `is_imputed_pm25`, `is_imputed_input` as trailing columns |
...
Reproduce this horizon with `python -m src.preprocessing.pipeline --horizon 1`,
