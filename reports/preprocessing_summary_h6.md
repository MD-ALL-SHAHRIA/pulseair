# Preprocessing summary -- horizon 6 h

> **This is the primary dataset.** Every later phase -- CTGAN, the sequence model, SHAP, the LLM advisory -- trains and reports on it.

Generated by `src/preprocessing/pipeline.py` from `configs/default.yaml`.
Source: `data/raw/beijing_multisite.csv` -> `data/processed/h6/`.

Window **24 h**, horizon **6 h**, split
...
| 5. windows -> train | 294,192 | shape (294192, 24, 9) |
| 5. windows -> val | 62,760 | shape (62760, 24, 9) |
| 5. windows -> test | 62,772 | shape (62772, 24, 9) |
...
| --- | --- | --- | --- | --- |
| `is_imputed` | label row (t+h) had any modelled channel forward-filled | 20,429 | 1,724 | 2,290 |
| `is_imputed_pm25` | label row had **PM2.5** forward-filled -- this sets the class | 6,160 | 1,294 | 1,242 |
| `is_imputed_input` | any of the 24 input hours was forward-filled | 91,664 (31.2%) | 14,795 (23.6%) | 14,970 (23.9%) |
...
PM2.5, of which **8,696** become the label row of a
sample (**24,499** / **24,443**
for any modelled channel). The gap is the
1,044 boundary rows per station per split that can never
...
prediction target (t + 6 h) of every sample.

| Class | Overall | train n (%) | val n (%) | test n (%) |
| --- | --- | --- | --- | --- |
| Good | 15.23% | 41,104 (13.97%) | 11,370 (18.12%) | 11,297 (18.00%) |
| Moderate | 22.03% | 63,989 (21.75%) | 14,292 (22.77%) | 14,250 (22.70%) |
| Unhealthy (sensitive) | 12.91% | 38,480 (13.08%) | 9,069 (14.45%) | 6,584 (10.49%) |
| Unhealthy | 34.55% | 104,898 (35.66%) | 20,942 (33.37%) | 19,216 (30.61%) |
| **Very unhealthy** | 10.72% | 33,032 (11.23%) | 4,405 (7.02%) | 7,629 (12.15%) |
| **Hazardous** | 4.56% | 12,689 (4.31%) | 2,682 (4.27%) | 3,796 (6.05%) |
...
29 boundary hours per station per split that cannot complete a
window (1,044 samples in total).
...
| `data/processed/tabular_<split>.csv` | same samples, flat -- the window's last step plus the t+6 h label; baseline model and CTGAN input. Carries `is_imputed`, `is_imputed_pm25`, `is_imputed_input` as trailing columns |
...
Reproduce this horizon with `python -m src.preprocessing.pipeline --horizon 6`,
